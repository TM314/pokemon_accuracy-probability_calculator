...
def calc(move_name_P='', move_name_O='', accuracy_stages_P='0', accuracy_stages_O='0', evasion_stages_P='0', evasion_stages_O='0', para_P=False, para_O=False, flinch_P=False, flinch_O=False, confusion_P=False, confusion_O=False, infatuation_P=False, infatuation_O=False, ability_P='', ability_O='', item_P='', item_O=''):
    ...
    if item_P == 'wide lens':
        item_mod_P *= 1.1
    if evasion_stages_O != '':
        evasion_stages_O = '-'+str(evasion_stages_O).replace("+", '')
    ...
    if item_O == 'wide lens':
        item_mod_O *= 1.1
    if evasion_stages_P != '':
        evasion_stages_P = '-'+str(evasion_stages_P).replace("+", '')

    if move_name_P != '':
        move_name_P = move_name_P.replace(" ", "-")
        move_P = pk.get_move(move_name_P)
        # print(accuracy_modifiers[str(accuracy_stages_P)], accuracy_modifiers[evasion_stages_O])
        final_acc_P = move_P.accuracy*accuracy_modifiers[str(accuracy_stages_P)]*accuracy_modifiers[evasion_stages_O]*ab_mod_P*item_mod_P

    if move_name_O != '':
        move_name_O = move_name_O.replace(" ", "-")
        move_O = pk.get_move(move_name_O)
        final_acc_O = move_O.accuracy*accuracy_modifiers[str(accuracy_stages_O)]*accuracy_modifiers[evasion_stages_P]*ab_mod_O*item_mod_O
    ...
    if flinch_P == True:
        ...
        if check_flinch > -1:
            ...
            if item_O == 'kings rock':
                ...
                mod_O = 1
            # print(move_O.accuracy, move_O.effect_chance)
            final_acc_P = (1-move_O.accuracy/100*mod_O*move_O.effect_chance/100)*final_acc_P/accuracy_modifiers[str(accuracy_stages_P)]
        else:
    ...
    if flinch_O == True:
        ...
        if check_flinch > -1:
            ...
            if item_P == 'kings rock':
                ...
                mod_P = 1
            final_acc_O = (1-move_P.accuracy/100*(mod_P*move_P.effect_chance/100))*final_acc_O/accuracy_modifiers[str(accuracy_stages_O)]
        else:
...
layout = [ [sg.Text('Player Move', size=(None,1)), sg.InputText('', size=(20,1), key='mnP'), sg.Text('Opponent Move'), sg.InputText('', size=(20,1), key='mnO')],
...
window = sg.Window(title='Pokemon Accuracy/Probability Calculator', layout=layout, auto_size_text=True, auto_size_buttons=True, resizable=True)
...
while True:
    ...
    if event == 'Submit':
        ...
        if len(result) == 2:
            if result[0] == None:
                sg.Popup('Probability = ', str(round(result[1],2)) + '%')
            elif result[1] == None:
